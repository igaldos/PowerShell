...
# .USAGE
#   Example:
#   Install-SentinelOne -SiteToken "<your-site-token>" -DownloadUrl "https://your-url/SentinelOneInstaller.exe" -Verbose
#   (DownloadUrl is optional if the installer already exists in the install directory)
#>
...
function Install-SentinelOne {
    ...
    Process {
        ...
        # Run installer
        $installerArgs = "--qn", "-t", $SiteToken
        Write-Verbose "Running: $installer $($installerArgs -join ' ')"
        $p = Start-Process -FilePath $installer -ArgumentList $installerArgs -PassThru -Wait
        if ($p.ExitCode -ne 0) {
